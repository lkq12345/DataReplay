...
| 实时数据来源 | 回放数据每行的属性字段（如 `x/y/z`，可配置） |
| 架构方案 | 方案 A：工作线程解析 + 增量信号 + 主线程节流刷新 |
| 规模 | 实体最多可能数千 → 严格性能设计 |
| **属性可扩展性** | 显示/跟踪的属性种类与个数**可配置**（属性名列表 + 动态列），后期加属性只改一处 |

## 三、总体架构（方案 A）

```
想定 XML ──只读解析──► EntityInfo(属性名→初始值) ──► 实体状态面板初始显示
                                                          ▲
...
struct EntityInfo {
    QString id;
    QString name;
    QMap<QString, double> attributes;   //!< 初始属性（属性名→值，来自想定 XML 的 Attribute，如 x/y/z）
};
```

### 4.2 `parseScenarioXml` 解析实体 Attribute

- 在现有 Entity ID/Name 解析基础上，读取 Entity 子元素 `<Attribute X="..." Y="..." Z="..." .../>` 的**数值属性**到 `EntityInfo.attributes`（按属性名列表提取，见 4.3）。
- **只读解析，不修改 XML**。

### 4.3 `ReplayWorker` 实时属性解析（性能核心）
...
struct EntityState {          //!< 运行时实体状态
    QString id;
    QMap<QString, double> attributes;   //!< 属性名→值（如 x/y/z，未来可扩展 speed/heading 等）
};
```

- **属性名列表集中配置**（唯一改动点）：
  ```cpp
  // ReplayWorker 内部维护"要跟踪/显示的属性名"（默认 x/y/z）
  QStringList m_trackedAttributes = { "x", "y", "z" };
  // 后期加属性：只需在此列表加 "speed" 等，其余逻辑（解析/表格列/更新）自动适配
  ```
- ReplayWorker 内部维护 `QHash<QString, EntityState> m_entityStates`。
- `processWindow()` 读取窗口数据后、**实体ID替换之前**，解析每条 `jsonPayload`：
  - 读取 `entity` 字段作为实体 ID；
  - 按 `m_trackedAttributes` 从 JSON 中提取各属性值，更新状态表；
  - 若**任一跟踪属性**发生变化，将该实体收集进"变化列表"（按 id 去重）。
- 窗口处理结束：`emit entityStatesUpdated(const QList<EntityState> &changed);`（每窗口一次）。
- 说明：解析在替换前进行，用**原始实体 ID** 与想定实体匹配，不受 mapping.json 映射影响。
- 想定初始属性与实时属性结构一致（都按属性名→值存储），便于面板初始填充与更新统一处理。
...
- 界面新增**实体状态面板**（`QTableView` + `QStandardItemModel`，只读）：
  - 列：实体ID | 名称 | **动态属性列**（表头由 `m_trackedAttributes` 生成，默认 X/Y/Z，后期加属性自动增加列）
...
- `QTimer`（100~200ms）`m_uiRefreshTimer` 触发 UI 刷新：
  ...
  - **按属性名定位对应列**（`属性名 → 列号` 映射），只对变化的属性格执行 `setItem`（增量更新源 model，代理/视图自动反映）。
- 搜索过滤时：源 model 全量存在，过滤后视图只显示匹配行，增量更新同样只改变化的行——天然满足"命中时只更新该实体"。

### 5.3 初始化填充与重置

- **初始化时**（`onInit()` 成功，状态进入 Ready）：用 `scenario->entities`（`attributes` 存初始值）填充实体状态表初始值，并按 `m_trackedAttributes` 生成属性列；同时建立 `id→行号` 与 `属性名→列号` 映射；清空 worker 的运行时状态表（`ReplayWorker::initialize` 时重置）。
...
| 场景 | 处理 |
|---|---|
| 数据行缺某跟踪属性或 `entity` 字段 | 跳过该字段，保留原值 |
...
1. 加载想定：实体状态面板显示全部实体的初始属性（默认 X/Y/Z，与 XML 一致）。
2. 搜索：输入实体ID/名称命中一条 → 面板只显示该实体且实时更新它；清空 → 恢复全部。
3. 回放实时性：回放时属性值随数据变化；倍速 100x、数千实体下界面不卡顿。
4. 面板开关：按钮显示/隐藏正常。
5. 停止/重初始化：状态重置为初始值。
6. **属性扩展验证**：修改 `m_trackedAttributes`（如加 `speed`）后，表格自动增加列、解析与更新自动生效，无需改其他代码。
7. 回归：现有映射配置、描述、回放功能不受影响。

## 十、涉及文件清单

| 文件 | 变更 | 说明 |
|---|---|---|
| `src/Server_DataReplay/ScenarioMgr.h` | 修改 | `EntityInfo` 改为 `QMap<QString,double> attributes`（属性名→初始值） |
| `src/Server_DataReplay/ScenarioMgr.cpp` | 修改 | `parseScenarioXml` 按属性名列表解析 Attribute 数值属性（只读） |
| `src/Server_DataReplay/ReplayWorker.h` | 修改 | 新增 `EntityState`（attributes 映射）+ `entityStatesUpdated` 信号 + `m_trackedAttributes` 配置 + 内部状态表 |
| `src/Server_DataReplay/ReplayWorker.cpp` | 修改 | `processWindow` 按跟踪属性名解析、维护状态表、发射变化列表 |
...
| `src/APP_DataReplay/DataReplayWidget.h/cpp` | 修改 | 实体状态面板（动态属性列）、搜索、节流刷新定时器、`onEntityStatesUpdated` 槽 |
| `src/APP_DataReplay/DataReplayWidget.ui` | 修改 | 新增实体状态面板、搜索框、"实体状态"开关按钮 |
| `src/APP_DataReplay/EntityStateFilterProxyModel.h/cpp` | 新增 | 实体状态表过滤代理（仿 `EntityFilterProxyModel`，按 ID/名称过滤） |
